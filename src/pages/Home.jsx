...
// small dynamic wrapper for quick editor so we don't always import it
const RoomQuickEditorWrapper = ({ selectedRoom, furnitureMap, setFurnitureMap, onClose, rooms = [], onSelectRoom }) => {
  const [Loaded, setLoaded] = React.useState(null);
  React.useEffect(() => {
    let mounted = true;
  import('../components/RoomQuickEditorNEW').then(mod => { if (mounted) setLoaded(() => mod.default); }).catch(() => {});
    return () => { mounted = false; };
  }, []);
  if (!Loaded) return null;
  const C = Loaded;
  return <C visible={true} onClose={onClose} selectedRoom={selectedRoom} furnitureMap={furnitureMap} setFurnitureMap={setFurnitureMap} rooms={rooms} onSelectRoom={onSelectRoom} />;
};
...
export default function Home() {
  ...
  // Per-item galleries (three images each) — separate images per design card
  const ITEM_GALLERIES = {
    // Interior: Modern Living Room, Luxury Kitchen, Master Suite — 2 curated images each
    'int-1': [
    ...
    // Exterior: facades / garden / evening — 2 curated images each
    'ext-1': [
    ...
    // Culture: community / craftsmanship / urban — 2 curated images each
    'cul-1': [
  ...
  const open3DViewer = (modelPath) => {
    ...
    // Add demo- prefix if not present
    if (!modelPath.startsWith('demo-')) {
      modelPath = `demo-${modelPath}`;
    }
      ...
      {selectedItem && (
        <div style={{
          ...
          <div style={{
            ...
            <button
              type="button"
              onClick={(e) => { e.stopPropagation(); e.preventDefault(); setSelectedItem(null); }}
              style={{
                position: 'absolute',
                top: 16,
                right: 16,
            ...
            <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
              ...
              <div style={{ padding: 32 }}>
                  ...
                  {show3DViewer && activeModel && (
                    <div style={{
                      ...
                      <div style={{
                        ...
                        <button
                          type="button"
                          onClick={(e) => { e.stopPropagation(); e.preventDefault(); setShow3DViewer(false); setActiveModel(null); }}
                          style={{
                            position: 'absolute',
                            top: 16,
                            right: 16,
                        ...
                        <ThreeDViewer modelPath={activeModel} isEditMode={isRoomEditMode} selectedRoomName={selectedRoomName} onSelectRoom={(name) => setSelectedRoomName(name)} externalFurniture={furnitureMap} onRoomsReady={(rooms) => setRoomsList(rooms)} />
                      </div>
                    </div>
                  )}
                <div style={{ display: 'flex', gap: 16, marginTop: 32 }}>
                  ...
                  {/* Replace the single 'View in 3D' button with a small image selector for the section */}
                  <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                    {(() => {
                      const sec = selectedItem && selectedItem.section ? selectedItem.section : null;
                      const singleImg = (selectedItem && ITEM_GALLERIES[selectedItem.id] && ITEM_GALLERIES[selectedItem.id][0]) ? ITEM_GALLERIES[selectedItem.id][0] : (sec && sampleData[sec] && sampleData[sec][0] ? sampleData[sec][0].img : (selectedItem ? selectedItem.img : null));
                      if (!singleImg) return null;
                      return (
                        <img src={singleImg}
                          alt={selectedItem ? selectedItem.title : 'image'}
                          title={selectedItem ? selectedItem.title : ''}
                          onClick={() => selectedItem && setSelectedItem({ ...selectedItem, section: sec || selectedItem.section, model3D: selectedItem.id ? `/models/${selectedItem.id}.glb` : selectedItem.model3D })}
                          style={{ width: 56, height: 40, objectFit: 'cover', borderRadius: 6, cursor: 'pointer', border: '1px solid rgba(0,0,0,0.06)' }}
                        />
                      );
                    })()}
                  </div>
  ...
  <div style={{ position: 'fixed', top: 16, right: 16, display: 'flex', flexDirection: 'column', gap: 8, alignItems: 'flex-end', zIndex: 1000 }}>
          ...
          <button
            onClick={() => setIsRoomEditMode(prev => !prev)}
            className="btn-soft"
            style={{ background: isRoomEditMode ? '#f44336' : '#4CAF50' }}
          >
            {isRoomEditMode ? 'Exit Room Editor' : 'Edit Room'}
          </button>
        </div>
        {isRoomEditMode && (
          // Lazy render the quick editor panel so it doesn't exist when not editing
          <React.Suspense fallback={null}>
            {/* RoomQuickEditor is a lightweight overlay to add furniture without changing core flows */}
            {/* Import on demand */}
            <RoomQuickEditorWrapper selectedRoom={selectedRoomName} furnitureMap={furnitureMap} setFurnitureMap={setFurnitureMap} onClose={() => setIsRoomEditMode(false)} rooms={roomsList} onSelectRoom={(name) => setSelectedRoomName(name)} />
          </React.Suspense>
        )}
        <button
          onClick={() => {
                  ...
                  <div style={{ padding: 20 }}>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      ...
                      <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                        ...
                        {/* Replace 'View in 3D' with a small image set for the section */}
                        <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                          {(() => {
                            const imgUrl = (ITEM_GALLERIES[item.id] && ITEM_GALLERIES[item.id][0]) || item.img;
                            return (
                              <img src={imgUrl} alt={item.title} title={item.title}
                                onClick={(e) => { e.stopPropagation(); setSelectedItem({ ...item, section: 'interior', model3D: `/models/${item.id}.glb` }); }}
                                style={{ width: 56, height: 40, objectFit: 'cover', borderRadius: 6, cursor: 'pointer', border: '1px solid rgba(0,0,0,0.06)' }} />
                            );
                          })()}
                        </div>
                      ...
                      <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                        ...
                        <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                          {(() => {
                            ...
                            return (
                              <img src={imgUrl} alt={item.title} title={item.title}
                                onClick={(e) => { e.stopPropagation(); setSelectedItem({ ...item, section: 'exterior', model3D: `/models/${item.id}.glb` }); }}
                                style={{ width: 56, height: 40, objectFit: 'cover', borderRadius: 6, cursor: 'pointer', border: '1px solid rgba(0,0,0,0.06)' }} />
                            );
                          })()}
                        </div>
                      ...
                      <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                        ...
                        <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                          {(() => {
                            ...
                            return (
                              <img src={imgUrl} alt={item.title} title={item.title}
                                onClick={(e) => { e.stopPropagation(); setSelectedItem({ ...item, section: 'culture', model3D: `/models/${item.id}.glb` }); }}
                                style={{ width: 56, height: 40, objectFit: 'cover', borderRadius: 6, cursor: 'pointer', border: '1px solid rgba(0,0,0,0.06)' }} />
                            );
                          })()}
                        </div>
